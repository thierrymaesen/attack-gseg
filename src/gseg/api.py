...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
      """Load ML models at startup and release them on shutdown.
          ...
          The retriever (BM25 + knowledge graph) and the semantic reranker
              are both loaded into the global ``models`` dict so that every
                  request handler can access them without re-loading.
                      """
