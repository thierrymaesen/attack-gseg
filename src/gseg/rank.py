...
(``all-MiniLM-L6-v2`` by default) to reorder BM25 candidate results
by cosine similarity to the query.  It also exposes a convenience
function ``combine_retrieval_rerank`` that wires the BM25 retriever
from Sprint 3 together with the reranker in a single call.
...
class RerankHit:
      """A single result after semantic reranking.
          ...
          Attributes:
                  technique_id: ATT&CK technique identifier (e.g. ``T1055``).
                          name: Human-readable technique name.
                                  description: Technique description text used for embedding.
                                          tactics: List of MITRE ATT&CK tactics for this technique.
                                                  url: URL to the ATT&CK technique page, if available.
                                                          bm25_score: Original BM25 relevance score.
                                                                  rerank_score: Cosine similarity score from the reranker.
                                                                          original_rank: Position in the original BM25 result list (1-based).
                                                                              """
...
class Reranker:
      """Semantic reranker using sentence-transformer embeddings.
          ...
          Loads a pre-trained sentence-transformer model and provides a
              ``rerank`` method that reorders candidate documents by cosine
    ...
    def rerank(
              ...
              """Rerank candidates by cosine similarity to *query*.
                      ...
                      Each candidate dict is expected to contain at least ``"name"``
                              and ``"description"`` keys whose values are concatenated to
                                      form the document text for embedding.  The returned dicts are
                                              copies of the originals enriched with a ``"rerank_score"`` key.

                                                      Args:
                                                                  query: Free-text search query.
                                                                              candidates: List of candidate dicts from BM25 retrieval.
                                                                                          top_k: Maximum number of results to return.

                                                                                                  Returns:
                                                                                                              Reranked list of candidate dicts, best first, each with an
                                                                                                                          added ``"rerank_score"`` float field.
                                                                                                                                  """
              if not candidates:
                            return []
        ...
        logger.debug(
                      "Encoded query + %d docs in %.0f ms", len(doc_texts), encode_ms
        )

        # --- cosine similarity ---
        similarities = util.cos_sim(query_embedding, doc_embeddings)[0]
        scores: np.ndarray = similarities.cpu().numpy() if hasattr(similarities, "cpu") else np.array(similarities)
    ...
    def model_info(self) -> Dict[str, Any]:
              """Return metadata about the loaded model.
                      ...
                      Returns:
                                  Dictionary with ``model_name``, ``device``, and
                                              ``embedding_dim`` keys.
                                                      """
              return {
...
def combine_retrieval_rerank(
      ...
      """Run BM25 retrieval then semantic reranking in a single call.
          ...
          Args:
                  retriever: An instance of ``RetrieverBM25`` (duck-typed; must
                              expose a ``search(query, top_k)`` method returning objects
                                          with ``technique_id``, ``name``, ``tactics``, ``url``, and
                                                                      ...
                                                                      query: Free-text search query.
                                                                              bm25_k: Number of candidates to retrieve with BM25.
                                                                                      final_k: Number of results to keep after reranking.
                                                                                          ...
                                                                                          Returns:
                                                                                                  Dictionary with ``query``, ``results`` (list of reranked dicts),
                                                                                                          ``bm25_candidates``, ``latency_ms``, and ``model_info`` keys.
                                                                                                              """
      t_start: float = time.monotonic()
    ...
    # --- convert TechniqueHit -> dict for reranker ---
    candidates: List[Dict[str, Any]] = []
...
DEMO_CANDIDATES: List[Dict[str, Any]] = [
      ...
      },
      {
                ...
                "description": (
                              ...
                              " telnet, SSH, and VNC. The adversary may then perform actions"
      ...
      },
      {
                ...
                "description": (
                              ...
                              " systems and are a common feature across many platforms."
                ),
                "tactics": ["execution"],
...
def main() -> None:
      ...
      parser.add_argument(
          ...
          help="Number of BM25 candidates (used with --use-retriever)",
      )
      parser.add_argument(
          "--data-dir",
          type=Path,
          default=None,
          help="Data directory for retriever (used with --use-retriever)",
      )
      parser.add_argument(
          "--use-retriever",
          action="store_true",
...
def _run_demo_mode(
      args: argparse.Namespace, reranker: Reranker, sep: str
) -> None:
          ...
          Args:
                  args: Parsed CLI arguments.
                          reranker: Initialised ``Reranker`` instance.
                                  sep: Separator string for CLI output.
                                      """
...
def _run_full_pipeline(
      ...
      try:
        ...
        logger.error(
                      "Could not import gseg.retrieve. "
                      "Ensure Sprint 3 module is available (poetry install)."
        )
        sys.exit(1)
