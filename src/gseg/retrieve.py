...
(``build_graph``), builds a BM25 index over technique nodes, and
exposes a search interface that returns ranked techniques together
with their related mitigations.
      ...
      """Lowercase, collapse whitespace, and strip a text string.
          ...
          Args:
                  text: Raw input text.
                      ...
                      Returns:
                              Cleaned text with single spaces and no leading/trailing whitespace.
                                  """
...
def tokenize(text: str) -> List[str]:
      """Tokenize text into a filtered list of terms.
          ...
          Applies ``normalize_text``, extracts tokens matching
              ``TOKEN_PATTERN``, then drops tokens shorter than 2 characters
                  and tokens present in the minimal stopword list.

                      Args:
                              text: Raw input text.

                                  Returns:
                                          List of filtered tokens suitable for BM25 indexing.
                                              """
                              ...
                              Dictionary mapping node IDs to normalised text strings.
                                  ...
                                  Raises:
                                          FileNotFoundError: If *path* does not exist.
                                                  ValueError: If the JSON content is not a ``dict[str, str]``.
                                                      """
      if not path.exists():
...
class RetrieverBM25:
      """BM25-based retriever over ATT&CK technique nodes.
          ...
          Loads the knowledge graph and text index produced by Sprint 2,
              builds a BM25Okapi index on technique text only, and provides
                  ``search`` and ``get_mitigations`` methods for downstream use.
                      ...
                      Args:
                              graph_path: Path to the pickled NetworkX DiGraph.
                                      text_index_path: Path to the ``text_index.json`` file.
                                          ...
                                          Raises:
                                                  FileNotFoundError: If graph or text index files are missing.
                                                          ValueError: If no technique nodes are found in the graph.
                                                              """
    ...
    def __init__(
        ...
        for node_id, attrs in self.graph.nodes(data=True):
                      ...
                      if node_type is None:
                                        logger.warning(
                                                              "Node %s has no 'type' attribute -- skipping", node_id
                                        )
                                        continue
    ...
    def get_node_attr(
              self, node_id: str, key: str, default: Any = None
    ) -> Any:
                      ...
                      Args:
                                  node_id: The node identifier.
                                              key: Attribute key to look up.
                                                          default: Value returned when the node or key is missing.
                      ...
                      Args:
                                  query: Free-text search query.
                                              top_k: Maximum number of results to return.
                                                      ...
                                                      Returns:
                                                                  Ranked list of ``TechniqueHit`` objects, best first.
                                                                          """
    ...
    def get_mitigations(
              self, technique_id: str, limit: int = 20
    ) -> List[Dict[str, Any]]:
              """Return mitigations linked to a technique via the knowledge graph.

                      Mitigations are the predecessor nodes of *technique_id* (edges
                              run mitigation -> technique). Results are sorted alphabetically
                                      by mitigation name and capped at *limit*.

                                              Args:
                                                          technique_id: ATT&CK technique identifier (e.g. ``T1055``).
                                                                      limit: Maximum number of mitigations to return.

                                                                              Returns:
                                                                                          List of dicts with keys ``mitigation_id``, ``name``,
                                                                                                      ``description``, and ``url``.
                                                                                                              """
        if technique_id not in self.graph:
                      logger.warning(
                          "Technique %s not found in graph", technique_id
        )
            return []
              ...
              """Return debug information about how a query is processed.
                      ...
                      Args:
                                  query: The raw search query string.
                                          ...
                                          Returns:
                                                      Dictionary with ``normalized_query``, ``tokens``, and
                                                                  ``token_count`` keys.
                                                                          """
...
def main() -> None:
    ...
    parser.add_argument(
              "--show-mitigations",
              action="store_true",
              help="Display mitigations for each technique hit",
    )
    parser.add_argument(
              "-v",
              "--verbose",
    ...
    try:
        ...
        # --- search ---
        hits: List[TechniqueHit] = retriever.search(
